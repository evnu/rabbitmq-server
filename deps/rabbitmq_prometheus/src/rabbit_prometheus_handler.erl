...
put_filtering_options_into_process_dictionary(Request) ->
    #{vhost := VHosts, family := Families, 'per-object' := PerObject} = cowboy_req:match_qs([{vhost, [], undefined}, {family, [], undefined}, {'per-object', [], undefined}], Request),
    ...
    case PerObject of
        <<"1">> ->
            put(prometheus_per_object, true);
        <<"true">> ->
            put(prometheus_per_object, true);
        _ -> ok
    end,
